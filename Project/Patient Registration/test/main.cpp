...
void LogFileData()
{
	...
	char FileName[10] = {"main.exe"};
	char File [50] = {"File size = "};
	char size;

	hFile = CreateFile("LogFile.dat",
						GENERIC_WRITE,
						0,
						NULL,
						CREATE_NEW,
    ...
    if(GetFileAttributesEx(FileName, 0, &fileInfo))
    {
        ...
        WriteFile(hFile, File, sizeof(File), &dwBytesWritten, NULL);
		WriteFile(hFile, size, sizeof(size), &dwBytesWritten, NULL);
    }
